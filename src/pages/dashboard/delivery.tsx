...
import { buildOrderDays, MAX_PAUSES_PER_MONTH, type OrderStatus } from "@/lib/subscription"
...
function Delivery() {
  ...
  const orderDays = useMemo(
    () => buildOrderDays(sub.startDate, sub.planDuration, sub.pausedDates),
    [sub.startDate, sub.planDuration, sub.pausedDates]
  )
  const upcoming = orderDays.filter((d) => d.status !== "Delivered").slice(0, 8)
  const slots = SLOTS_BY_MEALS_PER_DAY[sub.mealsPerDay]

  function handleToggle(date: string) {
    const result = togglePause(date)
      ...
      <div>
        ...
        <div className="space-y-3">
          {upcoming.map((day) => {
            ...
            const items = slots.map((slot) => defaultMenuFor(sub.goal, sub.dietType, sub.allergens, slot))
            return (
