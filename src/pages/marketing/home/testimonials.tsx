    ...
    <section className="py-20 sm:py-28 bg-cream-100">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="max-w-xl mx-auto text-center mb-14">
          <h2 className="text-3xl sm:text-4xl text-ink">What customers are saying</h2>
          <p className="mt-3 text-ink-muted">Real plans, real routines — a few notes from people on OlivePinch today.</p>
        </div>
        ...
        <div className="flex gap-5 overflow-x-auto pb-4 snap-x snap-mandatory -mx-5 px-5 sm:mx-0 sm:px-0 sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:overflow-visible">
          {TESTIMONIALS.map((t, i) => (
            <div
              key={t.name}
              className="min-w-[280px] sm:min-w-0 snap-start rounded-2xl bg-surface border border-border p-6 shadow-soft flex flex-col"
            >
              <div className="flex gap-0.5 mb-4" aria-label="5 out of 5 stars">
                {Array.from({ length: 5 }).map((_, star) => (
                  <Star key={star} className="h-4 w-4 fill-coral-500 text-coral-500" />
                ))}
              </div>
