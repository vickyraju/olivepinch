...
| OP-14 | Low | Data integrity | `activateSubscription` isn't fully idempotent — a duplicate `/payments/confirm` call after success 500s instead of returning cleanly (blocked by `PromoRedemption.subscriptionId` unique constraint, which is itself correct) | Confirmed in code | Benign double-click/retry produces a 500 instead of an idempotent 200 | **Fixed** — see `PAYMENT_SECURITY_AUDIT.md` (PS-02/PS-03), same pass also closed a related replay-reactivation gap on cancelled subscriptions |
...
- ~~Make `activateSubscription` fully idempotent (OP-14)~~ — done, see `PAYMENT_SECURITY_AUDIT.md`.
